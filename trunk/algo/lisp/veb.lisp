...
(defun values-max (veb)
  (values (max-key veb)
	  (max-val veb)))

(defun values-min (veb)
  (values (min-key veb)
	  (min-val veb)))
...
(defun to-list (veb initial next)
  (let ((lst nil))
    (do ((ptr (funcall next veb initial)
	      (funcall next veb (first ptr))))
	((null ptr))
      (push ptr lst))
    (nreverse lst)))


(defun to-asc-list (veb)
  (to-list veb -1 #'succ))

(defun to-desc-list (veb)
  (to-list veb (1+ (max-key veb)) #'pred))
