;;; =============================================
;;; https://www.spoj.pl/problems/BST/
;;;
	  ...
	  (cond ((and (null s) (null p))
		...
		((null p)
		 (let ((v (1+ (rest s))))
		   ...
		   (incf c v)))
		(t  
		 (let ((v (1+ (max (rest p) (rest s)))))
		   (incf k v)
		   (incf c v)))))
	c))) 


(defun run-stepper (&rest args)
  (let ((f (stepper 32)))
    (dolist (x args)
      (format t "~d~%" (funcall f x)))))


(defun test1 (n)
  (let ((f (stepper 32)))
    (dotimes (x n)
      (format t "~d => ~a~%" x (funcall f x)))))

(defun main ()
  (let ((f (stepper 32)))
    (dotimes (n (read))
      (let ((k (read)))
	(format t "~d~%" (funcall f k))))))
